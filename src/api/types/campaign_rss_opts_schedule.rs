...
pub struct CampaignRssOptsSchedule {
    /// The days of the week to send a daily RSS Campaign.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub daily_send: Option<CampaignRssOptsScheduleDailySend>,
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub weekly_send_day: Option<CampaignRssOptsScheduleWeeklySendDay>,
}

impl CampaignRssOptsSchedule {
    pub fn builder() -> CampaignRssOptsScheduleBuilder {
        <CampaignRssOptsScheduleBuilder as Default>::default()
    }
}

#[derive(Clone, PartialEq, Default, Debug)]
#[non_exhaustive]
pub struct CampaignRssOptsScheduleBuilder {
    daily_send: Option<CampaignRssOptsScheduleDailySend>,
    hour: Option<i64>,
    monthly_send_date: Option<f64>,
    weekly_send_day: Option<CampaignRssOptsScheduleWeeklySendDay>,
}

impl CampaignRssOptsScheduleBuilder {
    pub fn daily_send(mut self, value: CampaignRssOptsScheduleDailySend) -> Self {
    ...
    pub fn weekly_send_day(mut self, value: CampaignRssOptsScheduleWeeklySendDay) -> Self {
        self.weekly_send_day = Some(value);
        self
    }

    /// Consumes the builder and constructs a [`CampaignRssOptsSchedule`].
    pub fn build(self) -> Result<CampaignRssOptsSchedule, BuildError> {
        Ok(CampaignRssOptsSchedule {
